...
include_once 'classes/WorkExperience.php';
...
switch ( $method ) {
	case 'GET':
		if ( isset( $id ) ) {
			...
			$result = $workExperience->getSpecificWorkExperience( $id );
		} else {
			// If not, get all courses.
			$result = $workExperience->getAllWorkExperiences();
		}
	...
	case 'POST':
		if ($workExperience->checkToken($sent_token)) {
			...
			if ( $workExperience->createWorkExperience() ) {
	...
	case 'PUT':
		if ($workExperience->checkToken($sent_token)) {
			...
			if ( ! isset( $id ) ) {
				...
				$workExperience->startdate = $data->startdate;
				$workExperience->enddate   = $data->enddate;
				if ( $workExperience->updateWorkExperience( $id ) ) {
	...
	case 'DELETE':
		if ($workExperience->checkToken($sent_token)) {
			...
			if ( ! isset( $id ) ) {
				...
				if ( $workExperience->deleteWorkExperience( $id ) ) {
